...
**この経路はこのチェックポイントを読めない。** すでにDeepSeek独自の規約で
量子化されているためである:

* テンソル名がDeepSeekネイティブ形式(`layers.{L}.ffn.experts.{E}.w1.weight`)で、
  transformersが期待する `model.layers.*.mlp.*` ではない。層を組み立てるだけでも
  このリポジトリは明示的なリネーム表を持つ。
* 量子化はルーテッドexpertがFP4(E2M1、K方向32要素ブロック、E8M0スケール)、
  非expert部がFP8(E4M3、128×128ブロック)。compressed-tensorsが読む形式では
  ないのでロードには逆量子化が要り、通常の経路で保存すればREAPが触ってすら
  いない重みまで再符号化される。
